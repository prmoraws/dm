        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                    ...
                    <div wire:key="create-edit-modal" x-data="{ open: @json($isOpen) }" x-show="open"
                        ...
                        <div class="bg-gray-100 dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-md mx-4 p-6 md:p-8">
                            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">
                                {{ $bloco_id ? 'Editar Bloco' : 'Criar Bloco' }}
                            </h2>
                ...
                <div class="hidden md:block overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow">
                    <table class="w-full table-auto">
                        ...
                        <tbody class="text-gray-600 dark:text-gray-300 text-sm">
                            @forelse ($results as $index => $bloco)
                                <tr wire:key="bloco-{{ $bloco->id }}"
                                    ...
                                    <td class="py-3 px-6 text-left">{{ $bloco->id }}</td>
                                    <td class="py-3 px-6 text-left">{{ $bloco->nome }}</td>
                                    <td class="py-3 px-6 text-center">
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            <button wire:click="confirmDelete({{ $bloco->id }})"
                                            ...
                                            <button wire:click="teste({{ $bloco->id }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-3 px-6 text-center text-gray-500 dark:text-gray-400">
                                        Nenhum bloco encontrado para o termo "{{ $search }}".
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="md:hidden space-y-4">
                    @forelse ($results as $index => $bloco)
                        <div wire:key="bloco-card-{{ $bloco->id }}" x-data="{ show: true }" x-show="show"
                            x-transition:enter="transition ease-out duration-300"
