    ...
    <div
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div class="w-full md:w-1/3">
                        <input type="text" wire:model.live.debounce.300ms="search"
                            placeholder="Buscar por Nome, Bairro, Bloco..."
                    ...
                    <div wire:key="create-edit-modal" x-data="{ open: @json($isOpen) }" x-show="open"
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-300"
                            ...
                            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">
                                {{ $instituicao_id ? 'Editar Instituição' : 'Criar Instituição' }}
                            </h2>
                            <form wire:submit.prevent="store" class="space-y-6">
                                ...
                                <div class="flex justify-end gap-3 mt-4">
                                    ...
                                    <button type="submit"
                                        class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-5 rounded-lg transition-all duration-300 focus:outline-none focus:shadow-outline">{{ $instituicao_id ? 'Atualizar' : 'Criar' }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                @if ($isViewOpen && $selectedInstituicao)
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400"><strong>Telefone:</strong>
                                        {{ $selectedInstituicao->telefone ?: 'Não informado' }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400"><strong>Contato:</strong>
                                        {{ $selectedInstituicao->contato ?: 'Não informado' }}</p>
                                </div>
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400"><strong>Endereço:</strong>
                                        {{ $selectedInstituicao->endereco ?: 'Não informado' }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400"><strong>Bairro:</strong>
                                        {{ $selectedInstituicao->bairro ?: 'Não informado' }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        <strong>Link:</strong>
                                        @if ($selectedInstituicao->localização)
                                            <a href="{{ $selectedInstituicao->localização }}" target="_blank"
                    ...
                    <div wire:key="delete-modal" x-data="{ open: @json($confirmDeleteId) }" x-show="open"
                        ...
                        <div class="bg-gray-100 dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-md mx-4 p-6">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-300 mb-6">Tem certeza de que deseja excluir
                                esta instituição? Esta ação é irreversível!</p>
                            <div class="flex justify-end gap-3">
                                <button wire:click="$set('confirmDeleteId', null)"
                                    class="bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-300 font-semibold py-2 px-5 rounded-lg transition-all duration-150">Cancelar</button>
                                <button wire:click="delete"
                                    class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-5 rounded-lg flex items-center gap-2 transition-all duration-150 focus:outline-none focus:shadow-outline"><svg
                    ...
                    <table class="w-full table-auto">
                        <thead>
                            <tr
                                ...
                                <th class="py-3 px-6 text-left cursor-pointer" wire:click="sortBy('convidados')">
                                    Convidados @if ($sortField === 'convidados')
                                        <span>{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </th>
                                <th class="py-3 px-6 text-left cursor-pointer" wire:click="sortBy('onibus')">Ônibus
                                    @if ($sortField === 'onibus')
                                        <span>{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                    @endif
                                </th>
                        ...
                        <tbody class="text-gray-600 dark:text-gray-300 text-sm">
                            @forelse ($results as $instituicao)
                                <tr wire:key="instituicao-{{ $instituicao->id }}"
                                    ...
                                    <td class="py-3 px-6 text-left truncate max-w-xs"
                                        title="{{ $instituicao->nome }}">{{ $instituicao->nome }}</td>
                                    <td class="py-3 px-6 text-left truncate max-w-xs"
                                        title="{{ $instituicao->bairro }}">{{ $instituicao->bairro }}</td>
                                    <td class="py-3 px-6 text-left truncate max-w-xs"
                                        title="{{ $instituicao->convidados }}">{{ $instituicao->convidados }}</td>
                                    <td class="py-3 px-6 text-left truncate max-w-xs"
                                        title="{{ $instituicao->onibus }}">{{ $instituicao->onibus }}</td>
                                    <td class="py-3 px-6 text-left truncate max-w-xs"
                                        title="{{ $instituicao->bloco }}">{{ $instituicao->bloco }}</td>
                                    <td class="py-3 px-6 text-center">
                                        <div class="flex items-center justify-center space-x-3">
                                            <button wire:click="view({{ $instituicao->id }})"
                                            ...
                                            <button wire:click="edit({{ $instituicao->id }})"
                                            ...
                                            <button wire:click="confirmDelete({{ $instituicao->id }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-3 px-6 text-center text-gray-500">Nenhuma instituição
                ...
                <div class="md:hidden space-y-4">
                    @forelse ($results as $instituicao)
                        <div wire:key="instituicao-card-{{ $instituicao->id }}"
                            ...
                            <div class="flex justify-between items-start">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Bairro:
                                        {{ $instituicao->bairro }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Convidados:
                                        {{ $instituicao->convidados }}</p>
                    ...
                    @empty
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 text-center text-gray-500">
                            Nenhuma instituição encontrada.</div>
                    @endforelse
                </div>
