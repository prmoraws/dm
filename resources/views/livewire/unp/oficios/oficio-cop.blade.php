...
<div x-data="{
    ...
    viewOrDownload(oficioId) {
        if (this.isIos) {
           window.open('{{ route('cop.pdf.view', ['id' => 'TEMP_OFICIO_ID']) }}'.replace('TEMP_OFICIO_ID', oficioId), '_blank');
        } else {
            $wire.view(oficioId);
        }
    }
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div class="w-full md:w-1/3">
                        <input type="text" wire:model.live.debounce.300ms="searchTerm" placeholder="Buscar..."
                ...
                @if ($isOpen)
                    {{-- CORRIGIDO: Alinhamento e cores do modal --}}
                    <div wire:key="create-edit-modal"
                        ...
                        <div class="bg-gray-100 dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-2xl p-6">
                            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">
                                {{ $oficio_id ? 'Editar' : 'Criar' }} Ofício COP</h2>
                            <form wire:submit.prevent="store" class="space-y-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div><label for="data_oficio" class="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-300">Data do
                                            Ofício</label><input type="date" wire:model.defer="data_oficio"
                                            id="data_oficio" class="w-full rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600">
                                        @error('data_oficio')
                                            <span class="text-red-500 text-xs">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div><label
                                            class="block text-sm font-bold text-gray-400 dark:text-gray-500 mb-2">Presídio
                                            (COP)</label><input type="text" value="CENTRO DE OBSERVAÇÃO PENAL (COP)"
                                            class="w-full rounded-md bg-gray-200 dark:bg-gray-700 dark:text-gray-300 cursor-not-allowed" disabled></div>
                                    <div><label
                                            ...
                                            class="w-full rounded-md bg-gray-200 dark:bg-gray-700 dark:text-gray-300 cursor-not-allowed" disabled></div>
                                    <div><label for="evento" class="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-300">Evento</label><input
                                            type="text" wire:model.defer="evento" id="evento"
                                            class="w-full rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600">
                                        @error('evento')
                                            <span class="text-red-500 text-xs">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div><label for="unidade_id" class="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-300">Unidade do
                                            Evento</label><select wire:model.defer="unidade_id"
                                            class="w-full rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600">
                                    ...
                                    <div><label for="dia_hora_evento" class="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-300">Data e Hora
                                            do Acesso</label><input type="datetime-local"
                                ...
                                <div wire:ignore><label
                                        class="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-300">Convidados/Membros</label><x-advanced-select
                                        :options="$convidadoOptions" wire:model="convidados"
                                ...
                                <div class="flex flex-col max-h-9/10"> {{-- aqui fundo --}}
                                    ...
                                    <main class="h-[55rem] flex-grow pt-6">
                                        ...
                                        <div class="mt-6 space-y-4 text-justify text-sm leading-snug">
                                            <p class="indent-8 px-5">{!! $selectedOficio->evento_formatado !!}</p>
                                            <div>
                                                <p class="underline underline-offset-2 pl-5 pb-4">Abaixo segue os nomes
                                                    dos ofíciais e carros:</p>
                                                @foreach ($selectedOficio->lista_convidados as $convidado)
                                                    <ul class="list-disc font-semibold ml-12">
                                                        <li>{{ $convidado->nome }} - CPF:
                                                            {{ $convidado->cpf }}</li>
                                                    </ul>
                                                @endforeach
                                            </div>
                ...
                <div class="hidden md:block overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow">
                    <table class="w-full table-auto">
                        <thead class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 uppercase text-sm">
                            <tr>
                                <th class="py-3 px-6 text-left">Unidade do Evento</th>
                                <th class="py-3 px-6 text-left">Data de Acesso</th>
                        ...
                        <tbody class="text-sm">
                            @forelse($results as $index => $oficio)
                                <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 animate-slide-up"
                                    ...
                                    <td class="py-3 px-6 text-gray-700 dark:text-gray-300">{{ $oficio->unidade->nome ?? 'N/A' }}</td>
                                    <td class="py-3 px-6 text-gray-700 dark:text-gray-300">
                                        {{ \Carbon\Carbon::parse($oficio->dia_hora_evento)->format('d/m/Y H:i') }}</td>
                                    <td class="py-3 px-6 text-gray-700 dark:text-gray-300">{{ $oficio->evento }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-3 px-6 text-center text-gray-500 dark:text-gray-400">Nenhum ofício
                                        encontrado.</td>
                                </tr>
                            @endforelse
                        </tbody>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 animate-slide-up"
                            ...
                            <div class="flex justify-between items-start">
                                <div class="flex-1 space-y-1">
                                    <p class="text-lg font-bold text-gray-900 dark:text-gray-100">{{ $oficio->unidade->nome ?? 'N/A' }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ $oficio->evento }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Acesso:
                                        {{ \Carbon\Carbon::parse($oficio->dia_hora_evento)->format('d/m/Y H:i') }}</p>
                                </div>
                                <div class="flex flex-row items-center space-x-4 ml-4">
                                    {{-- BOTÃO ALTERADO --}}
                                    <button @click="viewOrDownload({{ $oficio->id }})" class="w-5 text-gray-500 dark:text-gray-400 hover:text-green-500"><svg
                                    ...
                                    <button wire:click="edit({{ $oficio->id }})" class="w-5 text-gray-500 dark:text-gray-400 hover:text-blue-500"><svg
                                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                        </svg></button>
                                    <button wire:click="confirmDelete({{ $oficio->id }})"
                                        class="w-5 text-gray-500 dark:text-gray-400 hover:text-red-500"><svg fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5-4h4M9 7v12m6-12v12" />
                                        </svg></button>
