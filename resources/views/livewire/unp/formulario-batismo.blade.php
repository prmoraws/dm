@section('title', 'Formulário de Batismo')
        ...
        <form wire:submit.prevent="salvar" class="space-y-6">
            ...
            <div>
                ...
                <select id="presidio_id" wire:model.defer="presidio_id"
                    @if (empty($bloco_id)) disabled @endif
                    ...
                    @foreach ($presidiosDisponiveis as $presidio)
                        <option value="{{ $presidio->id }}">{{ $presidio->nome }}</option>
                    @endforeach
                </select>
                @error('presidio_id')
                    <span class="text-red-500 text-xs mt-1 block font-medium">{{ $message }}</span>
